...
class GameSession:
    ...
    def __post_init__(self):
        ...
        try:
            llm_call = make_default_llm_call()
        except RuntimeError:
    ...
    def _check_llm_health(self) -> None:
        """Quick check that an Anthropic API key is available so the user sees
        an obvious error before the first turn fires (rather than a sentinel
        string in the log)."""
        if not self.agents:
            return
        ...
        if api_key:
            self.log.append(
                f"Anthropic API key found. Using model '{any_agent.model}' "
                f"for all 6 powers."
            )
        else:
            self.log.append(
                f"WARNING: ANTHROPIC_API_KEY is not set. AIs will fall back "
                f"to all-hold heuristics and won't speak. "
                f"Set the key in your shell with `export ANTHROPIC_API_KEY=...` "
