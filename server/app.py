...
def start():
    ...
    # Optional LLM provider selection (None/blank = auto-detect from env/Ollama).
    llm_kind = (data.get("llm_kind") or "").strip().lower() or None
    if llm_kind not in (None, "ollama", "anthropic", "openai", "google"):
        llm_kind = None
...
@app.route("/api/kg/analysis")
def api_kg_analysis():
    """Helix-style multi-channel analytics over the live minds: hubs, bridges,
    multi-channel trust edges. Read-only; safe to poll. Optional ?power=FRANCE."""
    if session is None:
        return jsonify({"error": "no session"}), 400
    import diplomacy_kg_analysis as KGA
    minds = {p: a.mind for p, a in session.agents.items() if hasattr(a, "mind")}
    if not minds:
        return jsonify({"error": "no substrate minds (raw_llm agents have none)"}), 200
    power = (request.args.get("power") or "").upper().strip()
    out = {"table": KGA.analyze_table(minds)}
    if power and power in minds:
        out["power"] = KGA.analyze_mind(minds[power])
    return jsonify(out)


@app.route("/api/llm/providers")
def api_llm_providers():
    """Which model providers are usable right now (for a New Game selector)."""
    import llm_providers
    avail = llm_providers.available()
    kind, model = llm_providers.resolve()
    return jsonify({"available": avail, "auto": {"kind": kind, "model": model},
                    "kinds": list(llm_providers.KINDS)})
