...
def make_default_llm_call(model: str = "claude-haiku-4-5-20251001"):
    """Returns an llm_call(prompt: str) -> str using the Anthropic SDK.

    Raises RuntimeError early if no API key, so the session can show a
    clear error in the log instead of failing on the first turn.
    """
    api_key = __import__("os").environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        raise RuntimeError(
            "ANTHROPIC_API_KEY not set; export it before launching")
    from run_v2 import make_anthropic_caller
    return make_anthropic_caller(model)
